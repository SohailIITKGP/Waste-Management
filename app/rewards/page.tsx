      ...
      try {
        ...
        if (userEmail) {
          ...
          if (fetchedUser) {
            ...
            // Use a Map to filter out duplicate rewards
            const uniqueRewardsMap = new Map<number, Reward>()
            fetchedRewards.forEach(reward => {
              if (!uniqueRewardsMap.has(reward.id)) {
                uniqueRewardsMap.set(reward.id, reward)
              }
            })
            const uniqueRewards = Array.from(uniqueRewardsMap.values()).filter(r => r.cost > 0)
